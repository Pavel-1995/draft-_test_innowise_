...
class SigninTest(APITestCase):
    ...
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='test1', password='test1',
                                                             email='test1@ex.com')
        self.user.save()
...
class SignInViewTest(APITestCase):
    ...
    def test_1_post(self):
        ...
        request = factory.post('http://127.0.0.1:8000/api/ticket/', json={'text_ticket': 'dont work',
                                                                     'user': 'test',
                                                                     'status': 1})
